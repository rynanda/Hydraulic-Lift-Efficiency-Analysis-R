...
AOVFIT_LU <- lm(LogInput ~ ExperimentE.df$Liquid.Viscocity)
summary(AOVFIT_LU)
...
AOVFIT_LC <- lm(LogInput ~ ExperimentE.df$Liquid.Contamination)
summary(AOVFIT_LC)
# Checking Independence, Constant Variance and Normality 
par(mfrow = c(2,2))
...
AOVFIT_TL <- lm(LogInput ~ ExperimentE.df$Tube.Length..mm.)
summary(AOVFIT_TL)
# Checking Independence, Constant Variance and Normality 
par(mfrow = c(2,2))
...
SRPA.w <- lm(data = ExperimentE.df, LogInput ~ Press.Area..mm.2.)
summary(SRPA.w)
plot(ExperimentE.df$Press.Area..mm.2.,LogInput)
...
LCPA.lm <- lm(LogInput ~ Press.Area..mm.2. + Liquid.Contamination, data = ExperimentE.df)
...
ancova_checkLCPA <- aov(LogInput ~ Press.Area..mm.2.*Liquid.Contamination, data = ExperimentE.df)
summary(ancova_checkLCPA)
## Testing For Unusual Observations(linearity)
X <- model.matrix(LogInput ~ Press.Area..mm.2. + Liquid.Contamination, data = ExperimentE.df)
k <- NCOL(X) - 1
n <- nrow(X)
...
LUPA.lm <- lm(LogInput ~ Press.Area..mm.2. + Liquid.Viscocity, data = ExperimentE.df)
## Linearity, independence, constant variance and normality checks
par(mfrow = c(2,3))
...
## Assumption check for the interaction
ancova_checkLUPA <- aov(LogInput ~ Press.Area..mm.2.*Liquid.Viscocity, data = ExperimentE.df)
summary(ancova_checkLUPA)
## Testing For Unusual Observations(linearity)
X <- model.matrix(LogInput ~ Press.Area..mm.2. + Liquid.Viscocity, data = ExperimentE.df)
k <- NCOL(X) - 1
n <- nrow(X)
# Creating the data frame of metrics for unusual observations
Unusual_Observations <- data.frame(leverage = hat(X),
...
## Fitting the linear model 
ExperimentE.df$Tube.Length..mm. <- as.factor(ExperimentE.df$Tube.Length..mm.)
TLPA.lm <- lm(LogInput ~ Press.Area..mm.2. + Tube.Length..mm., data = ExperimentE.df)
## Linearity, independence, constant variance and normality checks
par(mfrow = c(2,3))
...
## Assumption check for the interaction
ancova_checkTLPA <- aov(LogInput ~ Press.Area..mm.2.*Tube.Length..mm., data = ExperimentE.df)
summary(ancova_checkTLPA)
## Testing For Unusual Observations(linearity)
X <- model.matrix(LogInput ~ Press.Area..mm.2. + Tube.Length..mm., data = ExperimentE.df)
k <- NCOL(X) - 1
n <- nrow(X)
# Creating the data frame of metrics for unusual observations
Unusual_Observations <- data.frame(leverage = hat(X),
...
ANC_LCPA.acv <- aov(LogInput ~ Press.Area..mm.2. + Liquid.Contamination, data = ExperimentE.df)
summary(ANC_LCPA.acv)
ggplot(ExperimentE.df, aes(as.factor(Press.Area..mm.2.), LogInput)) +
...
ANC_LUPA.acv <- aov(LogInput ~ Press.Area..mm.2. + Liquid.Viscocity, data = ExperimentE.df)
...
ExperimentE.df$Tube.Length..mm. <- as.factor(ExperimentE.df$Tube.Length..mm.)
ANC_TLPA.acv <- aov(LogInput ~ Press.Area..mm.2. + ExperimentE.df$Tube.Length..mm., data = ExperimentE.df)
